...
def on_connect(client, userdata, flags, reason_code, properties):
    if reason_code == 0:
        ...
        client.subscribe(consts.DATA_TOPIC, qos=2)
        client.subscribe(consts.CAMERA_TOPIC, qos=2)
    else:
...
class SubHandler(object):
    ...
    async def handle_change(self, val):
        print(f"Raking in progress: {val[consts.LADLE_TILT_STATE_INDEX]}")
        process_ids = await self.ids_node.get_value()
        heat_id = int(process_ids[consts.HEAT_ID_INDEX])
        if val[consts.LADLE_TILT_STATE_INDEX]: # raking has stated
            # Publish to raking event topic that the raking has started and is in process
            event: dict = {"heat_id": heat_id, "state": val[consts.LADLE_TILT_STATE_INDEX]}
            self.mqttc.publish(consts.EVENT_TOPIC, json.dumps(event), qos=2)
...
async def main():
    ...
    mqtt_client.connect(consts.BROKER, consts.PORT)
            ...
            while True:
                try:
                    ...
                    try:
                        ...
                        cv_data: dict = {}
                        camera_data: dict = {}
                        if raking_data[0] == consts.DATA_TOPIC:
                            cv_data = raking_data[1]
                        elif raking_data[0] == consts.CAMERA_TOPIC:
                    ...
                    if cv_data:
                        ...
                        real_values[consts.HEAT_ID_INDEX] = float(cv_data["heat_id"])

                        real_values[consts.TIME_INDEX] = float(cv_data["total_time_seconds"])
                        real_values[consts.PULLS_INDEX] = float(cv_data["num_pulls"])
                        real_values[consts.STEEL_START_INDEX] = cv_data["overall"]["steel_pct_start"]
                        real_values[consts.STEEL_END_INDEX] = cv_data["overall"]["steel_pct_end"]
                        real_values[consts.TOTAL_SLAG_START_INDEX] = cv_data["overall"]["total_slag_pct_start"]
                        real_values[consts.TOTAL_SLAG_END_INDEX] = cv_data["overall"]["total_slag_pct_end"]
                        real_values[consts.SOLID_SLAG_START_INDEX] = cv_data["overall"]["solid_slag_pct_start"]
                        real_values[consts.SOLID_SLAG_END_INDEX] = cv_data["overall"]["solid_slag_pct_end"]
                        real_values[consts.LIQUID_SLAG_START_INDEX] = cv_data["overall"]["liquid_slag_pct_start"]
                        real_values[consts.LIQUID_SLAG_END_INDEX] = cv_data["overall"]["liquid_slag_pct_end"]
                        real_values[consts.SLAG_START_INDEX] = cv_data["overall"]["slag_index_start"]
                        real_values[consts.SLAG_END_INDEX] = cv_data["overall"]["slag_index_end"]
                        print(f"Raking for heat id {cv_data["heat_id"]} took {cv_data["total_time_seconds"]}s and required {cv_data["num_pulls"]} pulls")
                        cv_data = {}

                    elif camera_data:
                        real_values[consts.CAMERA_TEMPERATURE_INDEX] = camera_data["temperature"]
                        bool_values[consts.CAMERA_STATUS_INDEX] = camera_data["connected"]
                        camera_data = {}
